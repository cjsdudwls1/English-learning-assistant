...
export const TeacherPerformanceCard: React.FC<Props> = ({ teachers }) => {
  ...
  if (teachers.length === 0) {
    return (
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-3 sm:p-5">
        <h2 className="text-base sm:text-lg font-bold text-slate-800 dark:text-slate-200 mb-3 sm:mb-4">{t.director.teacherPerformance}</h2>
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-3 sm:p-5">
      <h2 className="text-base sm:text-lg font-bold text-slate-800 dark:text-slate-200 mb-1">{t.director.teacherPerformance}</h2>
      <p className="text-xs text-slate-500 dark:text-slate-400 mb-3 sm:mb-4">{t.director.teacherPerformanceHint}</p>
      <div className="overflow-x-auto">
        <table className="w-full text-xs sm:text-sm">
          <thead>
            <tr className="text-left text-slate-500 border-b border-slate-200 dark:border-slate-700">
              <th className="pb-2">{t.director.colName}</th>
              <th className="hidden pb-2 text-center sm:table-cell">{t.director.colClassCount}</th>
              <th className="hidden pb-2 text-center sm:table-cell">{t.director.colAssignmentCount}</th>
          ...
          <tbody>
            {teachers.map(tc => (
              <tr key={tc.userId} className="border-b border-slate-100 dark:border-slate-700/50">
                <td className="py-2 pr-2 break-words text-slate-700 dark:text-slate-300">
                  {tc.name || tc.email}
                  {tc.name && <span className="ml-1 text-xs text-slate-600 dark:text-slate-400">{tc.email}</span>}
                </td>
                <td className="hidden py-2 text-center text-slate-600 dark:text-slate-400 sm:table-cell">{tc.classCount}</td>
