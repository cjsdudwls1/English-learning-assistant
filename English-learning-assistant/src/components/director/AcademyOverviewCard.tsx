...
export const AcademyOverviewCard: React.FC<Props> = ({ overview }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-3 sm:p-5">
      <h2 className="text-base sm:text-lg font-bold text-slate-800 dark:text-slate-200 mb-3 sm:mb-4">{t.director.academyOverview}</h2>
...
const OverviewStat: React.FC<{ label: string; value: string; highlight?: boolean }> = ({ label, value, highlight }) => (
  <div className={`rounded-xl p-3 sm:p-4 text-center ${highlight ? 'bg-indigo-50 dark:bg-indigo-900/20' : 'bg-slate-50 dark:bg-slate-700/50'}`}>
    <p className="text-[11px] sm:text-xs text-slate-600 dark:text-slate-400">{label}</p>
