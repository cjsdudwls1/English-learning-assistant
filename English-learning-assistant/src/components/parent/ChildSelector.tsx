...
export const ChildSelector: React.FC<Props> = ({ children, selectedId, onSelect, onChildrenUpdate }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 space-y-4">
      <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200">{t.parent.myChildren}</h2>

      <div className="flex gap-2">
        <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder={t.parent.linkByEmailPlaceholder} className="flex-1 px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm" />
