...
export const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        ...
        setError(translateError(e, language, t, t.profile.loadError));
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(translateError(e, language, t, t.profile.saveError));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-6 md:p-8 border border-slate-200 dark:border-slate-700">
      ...
      {activeTab === 'profile' && <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label htmlFor="profile-grade" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t.profile.grade}</label>
          <select
            id="profile-grade"
            value={grade}
