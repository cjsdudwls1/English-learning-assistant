...
import { buildUserKeyClient, type UserKeyProvider } from "../_shared/providerClients.ts";
...
async function validateKey(provider: UserKeyProvider, apiKey: string): Promise<{ ok: boolean; error?: string }> {
  try {
    const client = buildUserKeyClient(provider, apiKey);
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === 'save') {
      ...
      // 유효성 1회 검증
      const v = await validateKey(provider, apiKey);
      if (!v.ok) {
        return errorResponse('API 키 검증 실패: 키가 올바른지 확인하세요', 400, v.error);
      }
      ...
      const { error } = await supabase
        .from('user_api_keys')
        .upsert(
          { user_id: userId, provider, encrypted_key: encrypted, key_hint: keyHint, is_active: true, updated_at: new Date().toISOString() },
          { onConflict: 'user_id,provider' },
        );
      if (error) return errorResponse('저장 실패', 500, error.message);

      return jsonResponse({ ok: true, provider, key_hint: keyHint, is_active: true });
    }
