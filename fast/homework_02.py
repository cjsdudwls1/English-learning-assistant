...
def get_price_quote(product_id: int):
    """상품 가격 견적 요청 - 세금 10% 포함한 세후 가격 계산"""
    if product_id not in products:
        raise HTTPException(status_code=404, detail="상품을 찾을 수 없습니다")
    
    product = products[product_id]
    base_price = product["가격"]
    tax_rate = 0.10  # 10% 세금
    ...
    return {
        ...
        "가격_견적": {
            "기본_가격": f"{base_price:,}원",
            "세금_율": f"{tax_rate*100}%",
            "세금_금액": f"{tax_amount:,}원",
            "최종_가격": f"{final_price:,}원"
        },
        "견적_요약": f"{product['상품명']} - 최종 가격: {final_price:,}원 (세금 포함)"
    }
