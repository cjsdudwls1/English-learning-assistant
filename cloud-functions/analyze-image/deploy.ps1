...
<#
analyze-image GCF 배포 스크립트 (Windows PowerShell)
- 30명 동시 부하 시나리오 대응:
  --memory=2GiB     이미지(sharp 리사이즈) + Gemini 응답 누적 OOM 방지
...
Write-Host "[deploy] 프로젝트 설정: $PROJECT_ID"
gcloud config set project $PROJECT_ID
if (-not $?) { throw 'gcloud config set project 실패' }

Write-Host "[deploy] 함수 배포 시작: $FUNCTION_NAME ($REGION)"
